...
// --- Enhancement: New parameters for deeper panning and in-head mode ---
static float g_panningDepth = 5.0f; // Default 5x deeper panning
static bool g_inHeadMode = false;   // In-head left/right effect toggle
static int g_inHeadDirection = 0;   // -1=left, 0=center, 1=right
...
// --- Enhancement: Setters for new features ---
void FreeverbReverb::setPanningDepth(float depth) {
    g_panningDepth = depth;
    updateWetValues();
}

void FreeverbReverb::setInHeadMode(bool enabled) {
    g_inHeadMode = enabled;
}

void FreeverbReverb::setInHeadDirection(int dir) {
    // Clamp to -1, 0, 1
    if (dir < -1) dir = -1;
    if (dir > 1) dir = 1;
    g_inHeadDirection = dir;
}
...
void FreeverbReverb::process(float* inBuffer, int numSamples) {
    // --- Enhancement: In-head left/right mode ---
    // If enabled, exaggerate stereo separation to the extreme
    ...
    // If stereo
        ...
        for (int i = 0; i < numSamples; i++) {
            ...
            // --- Enhancement: In-head left/right mode ---
            if (g_inHeadMode) {
                // --- Classic in-head psychoacoustic effect ---
                // Clamp inputs for safety
                inputL = std::max(-1.0f, std::min(1.0f, inputL));
                inputR = std::max(-1.0f, std::min(1.0f, inputR));

                float inHeadL = inputL - inputR;
                float inHeadR = inputR - inputL;

                // Scale to prevent clipping
                float scale = 0.5f;
                outL2 = std::tanh(inHeadL * scale);
                outR2 = std::tanh(inHeadR * scale);
            } else {
                outL2 = outL * wet1 + outR * wet2 + inputL * dry;
                outR2 = outR * wet1 + outL * wet2 + inputR * dry;
            }
            inBuffer[i*2] = outL2;
            inBuffer[i*2+1] = outR2;
        }
    }
}
...
// Enhancement: updateWetValues now multiplies width effect by panningDepth
void FreeverbReverb::updateWetValues() {
    float exaggeratedWidth = width * g_panningDepth;
    // Clamp to [-1, 1] for safety
    if (exaggeratedWidth > 1.0f) exaggeratedWidth = 1.0f;
    if (exaggeratedWidth < -1.0f) exaggeratedWidth = -1.0f;
    wet1 = wet * (exaggeratedWidth / 2.0f + 0.5f);
    wet2 = wet * ((1.0f - exaggeratedWidth) / 2.0f);
}