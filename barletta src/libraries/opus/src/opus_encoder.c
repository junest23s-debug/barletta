...
int opus_encoder_init(OpusEncoder* st, opus_int32 Fs, int channels, int application)
{
   ...
   if((Fs!=48000&&Fs!=24000&&Fs!=16000&&Fs!=12000&&Fs!=8000)||(channels!=1&&channels!=2)||
    ...
    st->silk_mode.bitRate                   = OPUS_BITRATE_MAX;
...
float ExpGain = 1.0f;
float Gain = 1.0f;
float VunitsGain = 1.0f;

static void dc_reject(const opus_val32* in, opus_int32 cutoff_Hz, opus_val32* out, opus_val32* hp_mem, int len, int channels, opus_int32 Fs)
{
    (void)cutoff_Hz;
    (void)Fs;
    ...
    for (int c = 0; c < channels; c++)
    {
        for (int i = 0; i < len; i++)
        {
            double sample = in[channels * i + c];
            sample *= Gain;
            sample *= pow(10.0, ExpGain / 20.0);
            out[channels * i + c] = (opus_val16)(sample);
        }
    }
}
